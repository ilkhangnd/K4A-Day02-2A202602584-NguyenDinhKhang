...
```text
Survey MOT xác nhận occlusion và ID switch là các thách thức có thật của bài toán tracking, nhưng không thay thế validation pain hoặc ROI trong domain video lợn. Nhóm nên build một workflow QA nhỏ: dùng rule về IoU/quỹ đạo/mất–tái xuất hiện để tạo candidate, bổ sung CLIP-ReID hoặc ReID có sẵn trong tracker để chấm điểm ngoại hình, rồi đưa top đoạn nghi vấn cho người xác nhận và sửa. SlowFast chưa phù hợp với MVP vì không giải trực tiếp bài toán identity và cần thêm dữ liệu/compute; nếu baseline ByteTrack/rule đã đạt ngưỡng thì không thêm AI scoring; TrackEval chỉ dùng để đo trên ground-truth subset và human review vẫn là boundary bắt buộc.
```
