...
| Survey / poll (evidence thứ cấp) | 2 nghiên cứu công bố; 40 annotator ở nghiên cứu tracking video và 1 user study về tagging video động vật | [Nghiên cứu annotation video tracking](https://doi.org/10.1609/hcomp.v11i1.27555) kết hợp telemetry và survey của 40 annotator, cho thấy độ phức tạp tác vụ làm thay đổi cách phân bổ thời gian và thời gian tự báo cáo có thể lệch thời gian quan sát. [User study về tagging video động vật](https://doi.org/10.1002/wlb3.01539) ghi nhận quy trình bán tự động xử lý 768 ± 83 giây so với 3.111 ± 269 giây khi làm tay (xấp xỉ nhanh hơn 4 lần); người dùng chấm mức dễ thực hiện 7,4 ± 0,9 so với 2,0 ± 0,3 trên thang 1–10. | Khác domain: vehicle tracking/camera trap, không phải QA ID switch cho video lợn. Các số trên không thể dùng làm baseline hoặc cam kết ROI của nhóm. | Dùng như bằng chứng thứ cấp cho pitch: video review/annotation có pain về thời gian và UX. Trước khi Go, nhóm vẫn chạy script ở mục 4.1a với 5–10 phản hồi hoặc 2 interview + bấm giờ trên video lợn. |
| Log / ticket / review (nếu có) | 1 self-report của data owner | “Sửa ID thì nhanh, nhưng để tìm đúng frame bắt đầu bị đổi ID mới mất thời gian, vì phải tua đi tua lại rất nhiều.” Đây là tín hiệu định tính để chọn candidate, không phải kết quả survey đại diện. | Với video ngắn hoặc ít đối tượng, kiểm tra thủ công vẫn có thể chấp nhận được; một nguồn self-report chưa đủ để suy ra pain hay ROI cho toàn bộ AI Engineer/annotator. | Thu hẹp actor sang người QA video dài, nhiều đối tượng, có overlap/occlusion; giữ quyết định Not Yet và yêu cầu thêm interview/bấm giờ trước khi build. |
...
| --- | --- | --- | --- | --- | --- |
| Survey học thuật về MOT | [Multiple Object Tracking in Deep Learning Approaches: A Survey](https://www.mdpi.com/2079-9292/10/19/2406) | Tổng quan các phương pháp MOT, benchmark và đánh giá; xác định occlusion và identity switch là hai vấn đề trung tâm của MOT. | Nguồn học thuật kiểm được, trực tiếp liên hệ giữa ID switch, appearance và data association; giúp kiểm tra rằng candidate của nhóm không phải một lỗi ngẫu nhiên chỉ có ở dataset nội bộ. | Survey tập trung phần lớn vào benchmark/đối tượng phổ biến, không đo thời gian QA annotation hoặc chứng minh hiệu quả trên video lợn. Không được suy diễn rằng một phương pháp đạt kết quả benchmark sẽ đạt ROI trong pilot. | Dùng nguồn này để giải thích vì sao occlusion/ID switch đáng nghiên cứu; vẫn phải lấy dữ liệu time-on-task và ground truth từ pilot để quyết định. |
...
| Nghiên cứu có survey người dùng về xử lý video động vật | [Improving manual tagging of camera-trap videos for wildlife studies](https://doi.org/10.1002/wlb3.01539) | So sánh quy trình gắn nhãn thủ công với công cụ hỗ trợ trực quan; có survey trải nghiệm của người dùng sau khi thực hiện tác vụ. | Trong nghiên cứu này, người dùng đánh giá tác vụ thủ công khó hơn đáng kể so với quy trình bán tự động (2,0 so với 7,4 trên thang 1–10); thời gian xử lý cũng giảm khoảng bốn lần mà số lỗi không khác biệt có ý nghĩa. Đây là bằng chứng gần với pain “xem video + gắn nhãn” hơn survey MOT thuần kỹ thuật. | Domain là camera trap động vật hoang dã và công cụ tagging, không phải QA ID switch cho đàn lợn. Cỡ mẫu, tác vụ và thiết kế công cụ khác nên không được lấy tỷ lệ 4× làm cam kết ROI của nhóm. | Dùng trong pitch như một case cho thấy workflow hỗ trợ người dùng có thể giảm tải video-review; pilot của nhóm vẫn cần bấm giờ trên chính video lợn và đo lỗi ID. |
| Nghiên cứu survey + telemetry về thời gian annotation video | [Characterizing Time Spent in Video Object Tracking Annotation Tasks](https://doi.org/10.1609/hcomp.v11i1.27555) | Phân tích thời gian làm tác vụ video-object-tracking; kết hợp telemetry và survey của 40 annotator toàn thời gian. | Cho thấy độ phức tạp của tác vụ ảnh hưởng riêng đến cách annotator phân bổ thời gian; thời gian tự báo cáo và thời gian quan sát có thể lệch nhau. Điều này trực tiếp ủng hộ việc thu cả survey lẫn log/bấm giờ trong pilot. | Đối tượng là tracking xe, không phải động vật; nghiên cứu không đo trực tiếp ID switch trên video lợn và không đưa ra một mức thời gian chung để áp vào nhóm. | Thiết kế validation của nhóm nên ghi timestamp từ lúc mở video đến lúc lưu, song song với câu trả lời survey; không chỉ hỏi “bạn thấy mất bao lâu?”. |
...
```text
Survey về MOT xác nhận rằng occlusion và ID switch là những vấn đề thật của bài toán tracking. Dù vậy, nó không thể thay cho việc kiểm chứng pain hoặc ROI trên video lợn của nhóm. Hướng nên thử là một workflow QA nhỏ: rule dùng IoU, quỹ đạo và sự kiện mất–tái xuất hiện để tạo candidate; CLIP-ReID hoặc ReID có sẵn trong tracker chỉ hỗ trợ chấm điểm ngoại hình; sau đó người dùng xem và quyết định sửa. SlowFast chưa phù hợp với MVP vì không giải trực tiếp bài toán identity và cần thêm dữ liệu/compute. Nếu ByteTrack hoặc rule đã đạt ngưỡng, nhóm sẽ không thêm AI scoring chỉ để bài trông “AI hơn”; TrackEval chỉ dùng để đo trên ground-truth subset và human review vẫn là boundary bắt buộc.
```
...
| Số bước thủ công | 6 bước (2–7) | 2 bước (review và sửa) | Đếm bước cần người thao tác; không tính bước máy chạy nhưng người chỉ khởi động. |
...
- [x] Có validation (quote thật) + research (link kiểm được) — có 1 quote self-report của data owner và 2 nghiên cứu có số liệu/link kiểm được; interview hoặc survey độc lập là bước tiếp theo để tăng độ chắc chắn cho pilot.
